...
class HiddenInstallerView extends Component {
  ...
  render () {
    ...
    const error = actionError && ((actionError.reason.errors && actionError.reason.errors[0]) || { detail: actionError.reason })
    return (
      <div role='contentinfo'>
        <div className='sto-content sto-hidden-install'>
          <h2 className='sto-installer-title'>{t('HiddenInstallerView.title')}</h2>
          <div className='coz-form'>
            <div className='sto-installer-field'>
              ...
              <input placeholder={t('HiddenInstallerView.source_placeholder')} type='text' name='source' onChange={(e) => this.onChange(e.target.name, e.target.value.trim())} />
            </div>
            <div className='sto-installer-field'>
              <h3>{t('HiddenInstallerView.slug')}</h3>
              <input type='text' name='slug' onChange={(e) => this.onChange(e.target.name, e.target.value)} />
            </div>
            <div className='sto-installer-field'>
              <h3>{t('HiddenInstallerView.update')}</h3>
              <Toggle id='isUpdate' checked={this.state.isUpdate} onToggle={() => this.onChange('isUpdate', !this.state.isUpdate)} />
            </div>
            {actionError &&
